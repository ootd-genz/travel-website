...
import { getPublicCatalog, getPublicDestination } from "@/lib/public/content";

type Props = { params: Promise<{ slug: string }> };
export async function generateMetadata({ params }: Props): Promise<Metadata> { const item = await getPublicDestination((await params).slug); if (!item) notFound(); return { title: item.seoTitle ?? item.name, description: item.seoDescription ?? item.shortDescription }; }

export default async function DestinationDetailPage({ params }: Props) {
  const slug = (await params).slug;
  const [destination, catalog] = await Promise.all([getPublicDestination(slug), getPublicCatalog()]);
  if (!destination) notFound();
  const trips = catalog.trips.filter((trip) => trip.destinations.some((item) => item.id === destination.id));
  const activityIds = new Set(trips.flatMap((trip) => trip.activities.map((item) => item.id)));
  const activities = catalog.activities.filter((item) => activityIds.has(item.id));
