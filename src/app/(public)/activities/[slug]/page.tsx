...
import { getPublicActivity, getPublicCatalog } from "@/lib/public/content";

type Props = { params: Promise<{ slug: string }> };
export async function generateMetadata({ params }: Props): Promise<Metadata> { const item = await getPublicActivity((await params).slug); if (!item) notFound(); return { title: item.seoTitle ?? item.name, description: item.seoDescription ?? item.shortDescription }; }

export default async function ActivityDetailPage({ params }: Props) {
  const slug = (await params).slug; const [activity, catalog] = await Promise.all([getPublicActivity(slug), getPublicCatalog()]); if (!activity) notFound();
  const trips = catalog.trips.filter((trip) => trip.activities.some((item) => item.id === activity.id));
  const destinations = catalog.destinations.filter((destination) => trips.some((trip) => trip.destinations.some((item) => item.id === destination.id)));
