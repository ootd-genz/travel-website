...
import { getPublicTrips, getPublicTripType, publicMediaUrl } from "@/lib/public/content";
import { createPublicMetadata } from "@/lib/seo";

type Props = { params: Promise<{ slug: string }> };
export async function generateMetadata({ params }: Props): Promise<Metadata> { const item = await getPublicTripType((await params).slug); if (!item) notFound(); return createPublicMetadata({ title: item.seoTitle ?? item.name, description: item.seoDescription ?? item.shortDescription ?? item.description.slice(0, 160), path: `/trip-types/${item.slug}`, image: publicMediaUrl(item.imagePath) }); }

export default async function TripTypeDetailPage({ params }: Props) {
  const slug = (await params).slug; const [tripType, allTrips] = await Promise.all([getPublicTripType(slug), getPublicTrips()]); if (!tripType) notFound();
  const trips = allTrips.filter((trip) => trip.tripTypes.some((item) => item.id === tripType.id));
